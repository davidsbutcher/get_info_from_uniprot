...
read_tdreport2 <- function(tdreport, fdr_cutoff = 0.01) {
  ...
  # Download SQL tables with needed data
  
  {
    globalqualconf <- con %>%
      ...
      as_tibble %>% 
      filter(ExternalId != 0) %>% 
      dplyr::rename("IsoformId" = ExternalId)
  ...
  output <- 
    left_join(isoform, globalqualconf) %>% 
  ...
  message("read_tdreport2 Finished!")
